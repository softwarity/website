...
:::tip
Noté que la base de données `mongodb` peut être clusterisée (dans swarm ou k8s), mais surtout peut être utilisé par vos microservices pour stocker vos documents. 
:::
...
:::tip
Notez que les données de mongodb ne sont pas persistées dans cet exemple.   
...
:::tip
Notez que en absence de certificat, le HTTPs n'est pas actif par defaut.
